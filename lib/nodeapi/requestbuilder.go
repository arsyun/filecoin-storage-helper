...
func NewRequestBuilder(command string, args []string, headers map[string]string, s API) *requestBuilder {
...
func (r *requestBuilder) Send(ctx context.Context) (*Response, error) {
	req := NewRequest(ctx, r.shell.GetURL(ctx), r.command, r.args...)
	req.Opts = r.opts
	req.Headers = r.headers
	req.Body = r.body

	return req.Send(r.shell.GetClient(ctx))
}
